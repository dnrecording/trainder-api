...
const getAllFriend_id = async(myId) => {
    ...
    FriendList.forEach((friend) => {
        if (friend.id != "Dummy") Friends_id.push(friend.id);
    });
    return Friends_id;
};
const getAllFriends = async(myId) => {
    ...
    for (let i = 0; i < friends_id.length; ++i) {
        let id = friends_id[i];
        let res = await db.collection("userData").doc(id).get();
        const data = res.data();
        Friends.push(new Friend(id, data.fullName, data.profilePic));
    }
    return Friends;
};
...
const getFriends_name = async(myId) => {
    ...
    const FriendList = await db
        ...
        .get();
    FriendList.forEach((friend) => {
        if (friend.id != "Dummy") Friends_name.push(friend.data().name);
    });
    return Friends_name;
};
module.exports = {
