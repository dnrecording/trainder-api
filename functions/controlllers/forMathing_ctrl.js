    ...
    try {
        ...
        if (data.empty) {
            ...
            data.forEach(doc => {
                ...
                const user = new M_user(doc.id, doc.data().EC_skill, doc.data().Purpose, doc.data().Birthday,met_list);
                if(user.genre == null || user.purpose==null||user.birth_day ==null){
                    db.collection('WaitForMatch').doc(user.id).delete();
                    console.log('Invalid  information for matching form user Id :'+ user.id);
